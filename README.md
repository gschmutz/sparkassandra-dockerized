...
```
# run a Spark master
docker run -d -t -P --name spark_master gschmutz/spark-cassandra /start-master.sh

# run a Cassandra + Spark worker node
docker run -it --name some-cassandra --link spark_master:spark_master -d gschmutz/spark-cassandra

# (optional) run some other nodes if you wish
docker run -it --link spark_master:spark_master --link some-cassandra:cassandra -d gschmutz/spark-cassandra
```
...
docker run -it --link some-cassandra:cassandra --rm gschmutz/spark-cassandra cqlsh cassandra
...
```
# run a Spark shell 
docker run -i -t -P --link spark_master:spark_master --link some-cassandra:cassandra gschmutz/spark-cassandra /spark-shell.sh
